...
func TestLoadAllProblemsSkipsTracks(t *testing.T) {
	dir := writeRegistryFixture(t)
	idx, err := LoadIndex(dir)
	require.NoError(t, err)
...
func TestLoadProviderManifestReturnsReadError(t *testing.T) {
	_, err := loadProviderManifest(filepath.Join(t.TempDir(), "missing.json"))
	require.Error(t, err)
}
...
func writeRegistryFixture(t *testing.T) string {
	...
	index := `{"version":"v1","updated_at":"2026-03-31T00:00:00Z","manifests":[` +
		`{"name":"leet","path":"providers/leet/free/problems.json"},` +
		`{"name":"roadmap","path":"tracks/base.json"}]}`
	provider := `{"problems":[` +
		`{"slug":"two-sum","title":"Two Sum","difficulty":"easy","provider":"leetcode",` +
		`"external_id":"1","tags":["array"],"pattern_tags":["hash-map"]}]}`
	...
	require.NoError(
		t,
		os.WriteFile(filepath.Join(dir, "providers", "leet", "free", "problems.json"), []byte(provider), 0o644),
	)
