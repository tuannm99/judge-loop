...
local function choose_language(problem, callback)
	local choices = language_pool(problem)
	vim.ui.select(choices, { prompt = "Language" }, function(language)
		if language then
			callback(language)
		end
	end)
...
function M.start_problem(problem)
	choose_language(problem, function(language)
		local starter = problem.starter_code or {}
		...
		require("judge-loop.agent").timer_start(problem.id, function(ok, data, err)
			if not ok then
			...
			M.info(
				"Solving " .. (problem.title or problem.slug or problem.id) .. " from " .. (data.started_at or "now")
			)
		end)
	end)
end
...
function M.switch_language(language)
	...
	vim.ui.select(language_pool(problem), { prompt = "Language" }, open_language)
end
...
function M.problem_actions(problem)
	vim.ui.select({ "Start solving", "Details", "Open source" }, {
...
function M.pick_problem()
	require("judge-loop.agent").problems({ limit = 200 }, function(ok, data, err)
		if not ok or not data or not data.problems then
			M.api_error("Could not load problems", err)
			return
		end
		vim.ui.select(data.problems, {
			prompt = "Problems",
			format_item = fmt_problem,
		}, function(problem)
			if problem then
				M.problem_actions(problem)
			end
		end)
	end)
end
...
function M.suggest_problem()
	require("judge-loop.agent").problem_suggest(function(ok, problem, err)
...
function M.sync_registry()
	require("judge-loop.agent").sync(function(ok, data, err)
		if not ok then
...
function M.dashboard()
	require("judge-loop.agent").status_today(function(status_ok, status, status_err)
		require("judge-loop.agent").timer_current(function(_, timer, timer_err)
